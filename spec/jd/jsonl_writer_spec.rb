...
RSpec.describe JD::JsonlWriter do
  ...
  describe "#open" do
    ...
    context "when a block is given" do
      ...
      it "yields the writer instance" do
        writer.open do |instance|
          expect(instance).to eq(writer)
        end
      end
  ...
  describe "#write" do
    ...
    let(:json) { JSON.dump(row) }

    it "writes rows as JSON" do
      writer.open.write(row)
      expect(file).to have_received(:write).with(json, "\n")
    end

    it "returns self" do
      expect(writer.open.write(row)).to eq(writer)
    end
  end
